...
class ThemeCubit extends Cubit<ThemeSettings> {
  /// Acento de la marca: el lavanda de la paleta Aurora, en su tono legible.
  static const String acentoPorDefecto = '#705BFF';

  /// El morado con el que nació la app. Se conserva solo para reconocerlo al
  /// migrar: quien lo tenga guardado no lo eligió —era el valor por defecto—,
  /// así que pasa al acento nuevo. A quien sí escogiera un color suyo no se le
  /// toca nada.
  static const String _acentoAntiguoPorDefecto = '#6750A4';

  ThemeCubit()
      : super(const ThemeSettings(
          modo:        ThemeMode.system,
          colorAcento: acentoPorDefecto,
        )) {
    _cargarPreferencias();
  }
  ...
  Future<void> _cargarPreferencias() async {
    ...
    var hexColor      = prefs.getString(_kAccentColorKey)  ?? acentoPorDefecto;

    // Sin esta migración, la paleta nueva no se vería en ningún dispositivo
    // que ya hubiera abierto la app: el morado quedó guardado la primera vez.
    if (hexColor.toUpperCase() == _acentoAntiguoPorDefecto) {
      hexColor = acentoPorDefecto;

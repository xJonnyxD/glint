...
class EventRepository {
  ...
  /// Actualiza los campos editables de un evento.
  Future<void> actualizarEvento(EventEntity e) async {
    await (_db.update(_db.events)..where((ev) => ev.id.equals(e.id))).write(
      EventsCompanion(
        titulo:      Value(e.titulo),
        descripcion: Value(e.descripcion),
        fecha:       Value(e.fecha),
        hora:        Value(e.hora),
        duracionMinutos: Value(e.duracionMinutos),

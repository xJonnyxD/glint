...
import 'package:glint/shared/widgets/sync_indicator.dart';
import 'package:glint/features/groups/domain/group_entity.dart';
import 'package:glint/features/groups/presentation/groups_cubit.dart';
import 'package:glint/features/groups/presentation/group_colors.dart';
            ...
            children: [
              ...
              BlocBuilder<GroupsCubit, GroupsState>(
                builder: (context, gState) {
                  if (gState is GroupsLoaded && gState.grupos.isNotEmpty) {
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Padding(
                        ...
                        Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          child: _TarjetaGrupos(grupos: gState.grupos),
                                ...
                                Row(
                                  children: [
                                    Container(
                                      ...
                                      child: Icon(gamState.iconoNivel,
    ...
    return Container(
      ...
      child: BlocBuilder<AuthCubit, GlintAuthState>(
        builder: (context, authState) {
          // El nombre sale del perfil (la fuente de verdad); el metadata de
          // auth queda solo como respaldo mientras el perfil carga.
          final perfilState = context.watch<ProfileCubit>().state;
          String nombre = 'Usuario';

          if (perfilState is ProfileLoaded) {
            nombre = perfilState.perfil.nombreCompleto.split(' ').first;
          } else if (authState is AuthAuthenticated) {
            final nombreMeta =
                authState.user.userMetadata?['nombre'] as String?;
            if (nombreMeta != null && nombreMeta.isNotEmpty) {
              nombre = nombreMeta.split(' ').first;
            }
          }
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  // Indicador de sincronización (spinner / al día / error)
                  const Padding(
                    padding: EdgeInsets.only(right: 12),
                    child: SyncIndicator(color: Colors.white70),
                  ),
                        ...
                        Container(
                          ...
                          child: BlocBuilder<ProfileCubit, ProfileState>(
                            builder: (context, perfilState) {
                              final perfil = perfilState is ProfileLoaded
                                  ? perfilState.perfil
                                  : null;
                              return AvatarGlint(
                                radio: 28,
                                ...
                                nombre: perfil?.nombreCompleto ?? nombre,
                                seed: perfil?.id,
                                colorFondo:
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            // Llama animada si hay racha >= 7, si no el ícono normal
            if (enRachaFuego)
              Icon(AppIcons.streak, size: 22, color: colorScheme.tertiary)
...
class _TarjetaFinanciera extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // El mismo interruptor que la pantalla de Finanzas: es un único
    // ValueNotifier, así que tapar el saldo aquí lo tapa también allí, como en
    // la app de un banco.
    return ValueListenableBuilder<bool>(
      valueListenable: PrivacidadSaldo.instancia,
      builder: (context, oculto, _) {
        // Con el importe tapado, dejar el verde o el rojo seguiría diciendo si
        // el mes va a favor o en contra, que es justo lo que se quiere esconder.
        final colorBalance = oculto
            ? colorScheme.onSurfaceVariant
            : (balancePositivo ? Colors.green.shade600 : Colors.red.shade600);
              ...
              children: [
                Row(
                  children: [
                    ...
                    // ancho en CUALQUIER móvil (medido: 320, 360 y 412 dp), y
                    // como la tarjeta recorta lo que sobresale, el ojo quedaba
                    // fuera —invisible pero todavía pulsable—. Así el título
                    // cede espacio con puntos suspensivos y el resto cabe.
                    Expanded(
                      child: Text(
                        'Balance del mes',
                    ...
                    IconButton(
                      ...
                      icon: Icon(
                        oculto ? AppIcons.hidden : AppIcons.visible,
                        size: 20,
                      ),
                      color: colorScheme.onSurfaceVariant,
                ...
                Text(
                  PrivacidadSaldo.instancia.formatear(fmt.format(state.balance)),
                  ...
                  children: [
                    Expanded(
                      child: _FilaFinanciera(
                        ...
                        // ingresos y los gastos a la vista se deduce.
                        monto: PrivacidadSaldo.instancia
                            .formatear(fmt.format(state.totalIngresos)),
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: _FilaFinanciera(
                        ...
                        label: 'Gastos',
                        monto: PrivacidadSaldo.instancia
                            .formatear(fmt.format(state.totalGastos)),
                      ),
                    ),
                  ],
                ),
...
class _AccionesRapidas extends StatelessWidget {
  // Cada acción lleva el color de su dominio; el tono concreto lo resuelve
  // AppColors.dominio() según el tema. Antes eran pasteles fijos que en modo
  // oscuro quedaban como bloques luminosos sobre el fondo oscuro.
  static const List<_AccionItem> _acciones = [
    _AccionItem(icono: AppIcons.routines,   label: 'Rutinas',     ruta: AppRoutes.routines,         dominio: Dominio.rutinas),
    _AccionItem(icono: AppIcons.habits,     label: 'Hábitos',     ruta: AppRoutes.habits,           dominio: Dominio.habitos),
    _AccionItem(icono: AppIcons.finance,    label: 'Finanzas',    ruta: AppRoutes.finance,          dominio: Dominio.finanzas),
    _AccionItem(icono: AppIcons.agenda,     label: 'Agenda',      ruta: AppRoutes.agenda,           dominio: Dominio.agenda),
    _AccionItem(icono: AppIcons.notes,      label: 'Notas',       ruta: AppRoutes.notes,            dominio: Dominio.meditacion),
    _AccionItem(icono: Symbols.handshake_rounded, label: 'Compartidos', ruta: AppRoutes.groups,           dominio: Dominio.finanzas),
    _AccionItem(icono: AppIcons.calculator, label: 'Calculadora', ruta: AppRoutes.salaryCalculator, dominio: Dominio.gamificacion),
  ];
  ...
  Widget build(BuildContext context) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Aparecer(indice: index, child: _BotonaAccion(accion: accion));
      },
    );
  }
}
...
class _BotonaAccionState extends State<_BotonaAccion>
  ...
  Widget build(BuildContext context) {
    ...
    final acento = AppColors.dominio(widget.accion.dominio, tema.brightness);

    // Tarjeta teñida con el color del dominio: un velo suave sobre la
    // superficie del tema, con borde del mismo tono. Así se distingue cada
    // acción sin dejar de pertenecer al tema claro u oscuro.
    final fondo = Color.alphaBlend(
      acento.withValues(alpha: oscuro ? 0.18 : 0.12),
      tema.colorScheme.surface,
    );
    final textoEtiqueta = AppColors.legibleSobre(acento, fondo);
    ...
    return ScaleTransition(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(18),
          // irA decide: las pestañas con `go`, el resto apilando con `push`.
          // Con `go` a secas, entrar aquí a "Compartidos" o a la calculadora
          // vaciaba la pila y el botón atrás cerraba la app.
          onTap: () => context.irA(widget.accion.ruta),
          onTapDown: _onTapDown,
          onTapUp: _onTapUp,
          onTapCancel: _onTapCancel,
          splashColor: acento.withValues(alpha: 0.16),
          highlightColor: acento.withValues(alpha: 0.08),
          child: Ink(
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(18),
...
// ── Tarjeta de gastos compartidos ──────────────────────────────────────────────

class _TarjetaGrupos extends StatelessWidget {
  final List<GroupEntity> grupos;
  const _TarjetaGrupos({required this.grupos});
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  const Icon(Symbols.handshake_rounded, size: 22),
                  const SizedBox(width: 8),
                  Text(
                ...
                children: [
                  for (final g in grupos.take(6))
                    Chip(
                      ...
                      label: Text(g.nombre),
                      backgroundColor: colorDesdeHex(g.color).withAlpha(28),
                      side: BorderSide(color: colorDesdeHex(g.color).withAlpha(90)),
                    ),
                ],
              ),
            ],

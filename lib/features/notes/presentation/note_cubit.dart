...
class NoteCubit extends Cubit<NoteState> {
  ...
  String _busqueda = '';
  CategoriaNote? _categoria;
  StreamSubscription? _sub;

  /// Dueño de estas notas. Lo necesita el editor para guardar su borrador.
  ...
  void cargarNotas() {
    ...
    _sub = _repo.watchNotas(_usuarioId).listen(
      (notas) => emit(NoteLoaded(notas,
          busqueda: _busqueda, categoriaFiltro: _categoria)),
  ...
  void buscar(String texto) {
    ...
    if (state is NoteLoaded) {
      emit(NoteLoaded(state.notas, busqueda: texto, categoriaFiltro: _categoria));
    }
  }

  /// Filtra por categoría (null = todas).
  void filtrarCategoria(CategoriaNote? categoria) {
    _categoria = categoria;
    final state = this.state;
    if (state is NoteLoaded) {
      emit(NoteLoaded(state.notas,
          busqueda: _busqueda, categoriaFiltro: categoria));
    }
  }

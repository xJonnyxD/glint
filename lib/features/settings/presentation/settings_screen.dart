...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _hacerCopia() async {
    ...
    try {
      final total = await BackupService.exportar(
        SyncManager.instance.baseDatos,
        authState.user.id,
      );
      if (!mounted) return;
      messenger.showSnackBar(SnackBar(
        content: Text('Copia creada con $total registros'),
        behavior: SnackBarBehavior.floating,
      ));
    } catch (e) {
      if (!mounted) return;
      messenger.showSnackBar(SnackBar(
        content: Text('No se pudo crear la copia: $e'),
        behavior: SnackBarBehavior.floating,
      ));
    }
  }
  ...
  Future<void> _restaurarCopia() async {
    // Confirmar: restaurar mezcla la copia con lo que ya haya.
    final confirmar = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Restaurar copia'),
        content: const Text(
            'Se añadirán los datos del archivo a los que ya tienes (los que '
            'coincidan se reemplazan). ¿Continuar?'),
        actions: [
          TextButton(
    ...
    try {
      final total = await BackupService.restaurar(
        SyncManager.instance.baseDatos,
        File(resultado.files.single.path!),
      );
      // Subir lo restaurado al servidor en segundo plano.
      if (authState is AuthAuthenticated) {
        SyncManager.instance.empujarEnSegundoPlano(authState.user.id);
      }
      if (!mounted) return;
      messenger.showSnackBar(SnackBar(
        content: Text('Copia restaurada: $total registros'),
        behavior: SnackBarBehavior.floating,
      ));
    } catch (e) {
      if (!mounted) return;
